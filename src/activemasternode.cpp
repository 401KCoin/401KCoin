...
void CActiveMasternode::ManageStatus()
{
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
        ...
        //     if (service.GetPort() != 5512) {
        //         notCapableReason = strprintf("Invalid port: %u - only 5512 is supported on mainnet.", service.GetPort());
        //         LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
        //         return;
        //     }
        // } else if (service.GetPort() == 5512) {
        //     notCapableReason = strprintf("Invalid port: %u - 5512 is only supported on mainnet.", service.GetPort());
        //     LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
        //     return;
        // }
...
bool CActiveMasternode::Register(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage)
{
    ...
    // if (Params().NetworkID() == CBaseChainParams::MAIN) {
    //     if (service.GetPort() != 5512) {
    //         errorMessage = strprintf("Invalid port %u for masternode %s - only 5512 is supported on mainnet.", service.GetPort(), strService);
    //         LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
    //         return false;
    //     }
    // } else if (service.GetPort() == 5512) {
    //     errorMessage = strprintf("Invalid port %u for masternode %s - 5512 is only supported on mainnet.", service.GetPort(), strService);
    //     LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
    //     return false;
    // }
